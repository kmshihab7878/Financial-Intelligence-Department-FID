...
1. **JSONL file**: Appended to `decision_log_path` (configured on Coordinator). One JSON object per line.
2. **Event store**: Appended as event_type `"decision"` via `EventStore.append_decision()`.
...
```python
# Recent decisions from event store
store = EventStore()
